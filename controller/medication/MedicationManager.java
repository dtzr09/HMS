...
/**
 * The MedicationManager class provides utility methods for managing medications
 * and its inventory.
 */
...
public class MedicationManager {
     ...
     * 
     * This method fetches a medication record from the database using its unique
     * medication ID. If the medication
     ...
     * @throws ModelNotFoundException If the medication with the provided ID does
     ...
     * 
     * This method adds a new medication record to the database. If the medication
     * already exists, a
     * `ModelAlreadyExistsException` is thrown.
     * 
     * @param medication The medication object to be added to the database.
     * @throws ModelAlreadyExistsException If a medication with the same ID already
     ...
     * 
     * This method updates the details of an existing medication in the database. If
     * the medication to be updated
     * does not exist, a `ModelNotFoundException` is thrown.
     * 
     * @param medication The medication object containing the updated details.
     * @throws ModelNotFoundException If the medication to update is not found in
     *                                the database.
     */
     ...
     * 
     * This method reads medication data from the file located at
     * `./resources/Medicine_List.csv`. For each row
     * in the CSV, it creates a new `Medication` object with the medication ID
     * (generated using `UUID`), name,
     * stock level, and low stock level alert. The method then attempts to add the
     * new medication to the database.
     * If the medication already exists in the database, a
     * `ModelAlreadyExistsException` is caught and an appropriate
     * message is printed.
     * 
     ...
     * 
     * @throws ModelAlreadyExistsException If a medication already exists in the
     *                                     database with the same ID.
     */
    public static void loadMedication() throws ModelAlreadyExistsException {
     ...
     * 
     * This method fetches and returns a list of all medications stored in the
     * medication database.
     * 
     * @return A list of `Medication` objects representing all medications in the
     *         database.
     */
    ...
    /**
     * Updates the stock level of a specific medication by increasing it by 10
     * units.
     * 
     * This method attempts to retrieve the medication by its ID and increase its
     * stock by 10 units.
     * If the medication is found, the stock is updated, and the `Medication` object
     * is saved back to the database.
     * If the medication with the given ID is not found in the database, an error
     * message is printed indicating
     * that the medication was not found.
     * 
     * @param medicationID The ID of the medication whose stock level is to be
     *                     updated.
     * @throws ModelNotFoundException If the medication with the specified ID is not
     *                                found in the database.
     */
    public static void updateMedicationStock(String medicationID) throws ModelNotFoundException {
     ...
     * 
     * This method attempts to retrieve the medication by its ID and decreases its
     * stock by 1 unit.
     * If the medication is found, the stock is updated, and the `Medication` object
     * is saved back to the database.
     * If the medication with the given ID is not found in the database, an error
     ...
     *                                found in the database.
     */
    public static void reduceMedicationStock(String medicationID) throws ModelNotFoundException {
        Medication medication = getMedicationsById(medicationID);
     ...
     * If the medication with the specified ID does not exist, an error message is
     * printed indicating
     * that the medication was not found.
     * 
     * @param medicationID The ID of the medication to be deleted.
     * @throws ModelNotFoundException If the medication with the specified ID does
     *                                not exist in the database.
     */
    public static void deleteMedication(String medicationID) throws ModelNotFoundException {
        MedicationDatabase.getDB().remove(medicationID);
    }

    /**
     * Retrieves a list of medications from the database based on a list of
     * medication IDs.
     * 
     * This method takes a list of medication IDs and attempts to fetch the
     * corresponding medication
     * objects from the database. If a medication ID does not exist in the database,
     * a ModelNotFoundException
     * will be thrown for that specific medication ID.
     * 
     * @param medicationIDs The list of medication IDs for which the medications
     *                      need to be fetched.
     * @return A list of Medication objects corresponding to the provided IDs.
     * @throws ModelNotFoundException If any medication ID is not found in the
     *                                database.
     */
     ...
     * This method takes a list of medication IDs and attempts to fetch the
     * corresponding medication
     * objects from the database. It then collects and returns the names of these
     * medications.
     * If any medication ID does not exist in the database, an error message is
     * printed for that ID.
     * 
     * @param medicationIDs The list of medication IDs for which the medication
     *                      names need to be fetched.
     * @return A list of medication names corresponding to the provided medication
     *         IDs.
     */
    ...
    /**
     * Retrieves the IDs of medications that are at or below their low stock level
     * alert.
     * 
     * This method checks all medications in the database and compares their current
     * stock levels
     * with their defined low stock level alert. If a medication's stock is at or
     * below the alert level,
     * its ID is added to the list.
     * 
     * @return A list of medication IDs for those that are at or below the low stock
     *         level alert.
     */
    ...
    /**
     * Retrieves a list of medications that are at or below their low stock level
     * alert.
     * 
     * This method checks for medications whose stock levels are at or below the low
     * stock level
     * threshold and returns a list of these medications.
     * It uses the medication IDs retrieved from `getLowStockIDs` and attempts to
     * fetch the corresponding
     * `Medication` objects. If any medication is not found, the exception is caught
